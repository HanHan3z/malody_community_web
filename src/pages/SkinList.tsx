...
function SkinListPage() {
  ...
  const buildParams = (current: FilterState, from: number) => {
    const params: { mode?: number; word?: string; from?: number } = { mode: current.mode, from }
    ...
    <div className="page">
      ...
      <section className="section">
        <div className="section-header">
          ...
          <span className="chart-count">{error ? error : t('skins.results.count', { count: skins.length })}</span>
        </div>
        {skins.length ? (
              ...
              <a className="chart-card skin-card" href={`/skin/${item.id}`} key={item.id}>
                <div className="chart-card-cover" style={{ backgroundImage: `url(${item.cover})` }}>
                  <div className="chart-card-badges">
                    ...
                    {item.isBought && <span className="pill ghost">{t('skins.badge.owned')}</span>}
                  </div>
                </div>
                  ...
                  <div className="chart-card-header">
                    <div>
                      ...
                      <p className="chart-card-artist">
                        {item.creator ? t('skins.creator', { name: item.creator }) : t('skins.creator.unknown')}
                      </p>
                    </div>
                  </div>
                  <div className="chart-card-meta">
                    <span className="meta-pill">{formatPrice(item.price)}</span>
                    {item.sales !== undefined && item.sales !== null && <span className="meta-pill">{formatSales(item.sales)}</span>}
                  </div>
                  <div className="chart-card-footer">
                    <span className="chart-card-updated">{formatUpdated(item.time)}</span>
                  </div>
                </div>
              </a>
            ))}
          ...
          <div className="chart-empty">{error ? error : loading ? t('skins.loading') : t('skins.results.empty')}</div>
        )}
        <div className="chart-actions">
          {hasMore && (
            <button className="load-more" type="button" onClick={() => loadSkins()} disabled={loading}>
