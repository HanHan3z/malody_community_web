...
const parseSkinId = () => {
  const detailMatch = window.location.pathname.match(/\/store\/skin\/detail\/(\d+)/)
    ...
    const loadSkin = async () => {
      ...
      try {
        ...
        const mapped: SkinDetailInfo = {
          ...
          previewImages: item.preview ? item.preview.split('|').filter(Boolean).map((img) => coverUrl(img)) : [],
      ...
      <header className="skin-detail-hero content-container">
        ...
        <div className="skin-summary">
          ...
          <p className="skin-creator">
            {skin?.creator ? t('skinDetail.creator', { name: skin.creator }) : t('skinDetail.creator.unknown')}
          </p>
          <div className="skin-meta-row">
            ...
            {skin?.sales !== undefined && <span className="pill ghost">{t('skinDetail.sales', { value: skin.sales })}</span>}
            {skin?.time && <span className="pill ghost">{t('skinDetail.updated', { time: formatUpdated(skin.time) })}</span>}
            {skinId && <span className="pill ghost">{t('skinDetail.meta.id', { id: skinId })}</span>}
          </div>
      ...
      <main className="content-container">
        <section className="skin-purchase-card">
          <div>
            ...
            <p className="skin-price-hint">
              {skin?.price ? t('skinDetail.purchase.desc') : t('skinDetail.purchase.freeDesc')}
            </p>
          </div>
          <div className="skin-purchase-actions">
            <button className="btn primary" type="button" onClick={handleBuy} disabled={purchaseLoading || !skinId}>
              {purchaseLoading
            ...
            {downloadReady && (
              <a className="btn ghost" href={purchaseData?.url} target="_blank" rel="noreferrer" download={purchaseData?.name}>
        ...
        <section className="section">
          <div className="section-header">
            ...
            {skin?.previewImages.length ? (
              <span className="chart-count">{t('skinDetail.preview.count', { count: skin.previewImages.length })}</span>
            ) : null}
          </div>
          {skin?.previewImages.length ? (
            <div className="skin-preview-grid">
              {skin.previewImages.map((img, idx) => (
                <div className="skin-preview-item" key={`${img}-${idx}`}>
                  <img src={img} alt={t('skinDetail.preview.alt', { index: idx + 1 })} />
                </div>
              ))}
            </div>
          ) : (
            <div className="skin-empty">{loadingInfo ? t('skins.loading') : t('skinDetail.preview.empty')}</div>
          )}
        </section>
        ...
        <section className="section">
          ...
          {!wikiLoading && !wikiError && (
            <>
              {templateError && <div className="skin-error">{templateError}</div>}
              {wikiHtml ? (
