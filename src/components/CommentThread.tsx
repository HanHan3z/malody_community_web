    ...
    <div className="comment-thread">
      <form className="comment-editor" onSubmit={onSubmit}>
        ...
        <div className="comment-editor-actions">
          {sessionUid && <span className="comment-hint">{t('comment.currentUser', { uid: sessionUid })}</span>}
      ...
      {!loading && comments.length === 0 && !error && <div className="comment-empty">{t('comment.empty')}</div>}
      ...
      {hasMore && (
        <div className="comment-actions">
          <button className="btn ghost small" type="button" onClick={() => loadComments(false)} disabled={loadingMore}>
